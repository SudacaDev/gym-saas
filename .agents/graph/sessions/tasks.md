...
- [x] Alta de staff: usuario+contraseña definidos por el owner (reemplaza invitación por email)
    ...
    - gate: `graph/gates/pending/T-20260825-002.md` — **`high`, síncrono, requiere aprobador nombrado — sin aprobar todavía.** Señala confirmar que `staff_members` tiene 0 filas reales en producción antes de aprobar (columna `username` se agrega `NOT NULL`). `npm run db:migrate` no corrido.
    - commit: `3fae426`
...
- [x] Check-in: ID visual corto para socios (`short_code`)
    ...
    - gate: `graph/gates/pending/T-20260825-003.md` — `medium`/async (subido de `low` por el backfill de filas reales). `npm run db:migrate` no corrido — compartirá el mismo comando que T-002/T-004 cuando se apruebe.
    - commit: `77064f4`
    - depends_on: ninguna
...
- [x] Check-in manual por código de 6 dígitos (auto-check-in, `checkin_code`)
    ...
    - gate: `graph/gates/pending/T-20260825-004.md` — `medium`/async. `npm run db:migrate` no corrido — compartirá el mismo comando que T-002/T-003 cuando se apruebe.
    - commit: `62a1aa1`
    - depends_on: ninguna
